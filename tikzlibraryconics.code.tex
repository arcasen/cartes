\ProvidesFile{tikzlibraryconics.code.tex}[2026/06/09 v1.2.5 A tikz library for conics]
...
\tikzset{
  ...
  % exclude coorindate
  % conics/exclude={\A,\B,\C,\D}
  % if A != C, result D = A
  % if B != C, result D = B
  % else throw pgferror
  conics/equal/.code args={#1,#2,#3}{
    \tikzmath{
      if abs((#1{1,1})*(#2{3,1})-(#2{1,1})*(#1{3,1})) < 1000*\EPSILON
          && abs((#1{2,1})*(#2{3,1})-(#2{2,1})*(#1{3,1})) < 1000*\EPSILON 
          && abs((#1{1,1})*(#2{2,1})-(#2{1,1})*(#1{2,1})) < 1000*\EPSILON then {
        #3 = 1;
      } else {
        #3 = 0;
      };
  ...
  conics/exclude/.code args={#1,#2,#3,#4}{
    \tikzset{
      conics/equal={#1,#3,\cs@firstequal},
      conics/equal={#2,#3,\cs@secondequal},
    }
    \tikzmath{
      if \cs@firstequal == 0 then {
        #4{1,1} = #1{1,1}; #4{2,1} = #1{2,1}; #4{3,1} = #1{3,1};
      } else {
        if \cs@secondequal == 0 then {
          #4{1,1} = #2{1,1}; #4{2,1} = #2{2,1}; #4{3,1} = #2{3,1};
        } else {
  ...
  conics/dehomogenize/.code args={#1,#2}{
    \tikzmath{
      if abs(#1{3,1}) <= \EPSILON then {
        ...
        #2{1} = \fpeval{#1{1,1}/#1{3,1}};
        #2{2} = \fpeval{#1{2,1}/#1{3,1}};
      };
    }
  },
  ...
  % midpoint of a segment in homogeneous cooordinates
  conics/midpoint/.code args={#1,#2,#3}{
    \tikzset{
      mat/combine={#1,#2,3,1,(#2{3,1}),(#1{3,1}),#3},
    }
  },
  % cross product of two vector (homogeneous coordinates), normalized
  % conics/cross={\A,\B,C}
  % C = A x B
  conics/cross/.code args={#1,#2,#3}{
    \tikzmath{
      \cs@cr{1,1}=\fpeval{#1{2,1}*#2{3,1}-#1{3,1}*#2{2,1}};
      \cs@cr{2,1}=\fpeval{#1{3,1}*#2{1,1}-#1{1,1}*#2{3,1}};
      \cs@cr{3,1}=\fpeval{#1{1,1}*#2{2,1}-#1{2,1}*#2{1,1}};
    }
    \tikzset{
      % Note: the cross product of two identical
      % homogeneous coordinates is the zero vector. 
      mat/normalize={\cs@cr,3,1,#3},
    }
  },
  ...
  conics/x-intercept/.code args={#1,#2}{
    \tikzmath{
      % y = 0
      \cs@xaxis{1,1} = 0; \cs@xaxis{2,1} = 1; \cs@xaxis{3,1} = 0; 
    }
    \tikzset{
  ...
  conics/y-intercept/.code args={#1,#2}{
    \tikzmath{
      % x = 0
      \cs@yaxis{1,1} = 1; \cs@yaxis{2,1} = 0; \cs@yaxis{3,1} = 0; 
    }
    \tikzset{
  ...
  conics/define/circle/center-radius/.code args={#1,#2,#3}{
    \tikzmath{
      if abs(#1{3,1}) < \EPSILON then {
        {\pgferror{Circle center cannot be ideal point}};
      };
      if #2 < \EPSILON then {
        {\pgferror{Circle radius cannot be negative}};
      };
      \cs@xc = \fpeval{#1{1,1}/#1{3,1}};
      \cs@yc = \fpeval{#1{2,1}/#1{3,1}};
      #3{1,1} = 1;       #3{1,2} = 0;       #3{1,3} = -\cs@xc;
      #3{2,1} = 0;       #3{2,2} = 1;       #3{2,3} = -\cs@yc;
      #3{3,1} = -\cs@xc; #3{3,2} = -\cs@yc; #3{3,3} = \fpeval{(\cs@xc)^2+(\cs@yc)^2-(#2)^2};
    }
  },
  % conics/define/circle/@center-point={\Center,\P,\C}
  conics/define/circle/@center-point/.code args={#1,#2,#3}{
    \tikzset{
      conics/distance={#2,#1,\cs@radius},
      conics/define/circle/center-radius={#1,\cs@radius,#3},
    }
  },
  ...
  conics/define/circle/.code={% dispatcher
    ...
    \ifnum \pgfkeysvalueof{/tikz/conics/define/circle/@argn} = 3
      \tikzset{conics/define/circle/@center-point= {#1}}
  ...
  conics/define/focus-directrix/.code args={#1,#2,#3,#4}{
    \tikzmath{
      % eccentricity e > 0
      if #3 < \EPSILON then {
        {\pgferror{Attempt to define a conic with e <=0}};
      };
      if abs(#1{3,1}) < \EPSILON then {
        {\pgferror{Focus cannot be ideal point}};
      } else {
        % dehomogenize
        \cs@fx = \fpeval{#1{1,1}/#1{3,1}};
        \cs@fy = \fpeval{#1{2,1}/#1{3,1}};
      };
      \cs@P{1,1} = 1;         \cs@P{1,2} = 0;         \cs@P{1,3} = -(\cs@fx);
      \cs@P{2,1} = 0;         \cs@P{2,2} = 1;         \cs@P{2,3} = -(\cs@fy);
      \cs@P{3,1} = -(\cs@fx); \cs@P{3,2} = -(\cs@fy); \cs@P{3,3} = (\cs@fx)^2+(\cs@fy)^2;
      \cs@scale = \fpeval{-(#3)^2/((#2{1,1})^2+(#2{2,1})^2)};
    }
    \tikzset{
      mat/outer product={#2,#2,3,\cs@T},
      mat/combine={\cs@P,\cs@T,3,3,1,\cs@scale,#4},
    }
  },
  % bi-focal definition of ellipse or hyperbola
  % conics/define/foci-axis={\Fa,\Fb,\a,\C}
  ...
  % \C-conic matrix
  conics/define/foci-axis/.code args={#1,#2,#3,#4}{
    \tikzset{
      % compute center in homogeneous coordinates
      % mat/combine={#1,#2,3,1,(#2{3,1}),(#1{3,1}),\cs@center},
      conics/midpoint={#1,#2,\cs@center},
      % mat/stdout={\cs@center,3,1},
      % 焦距 Focal Distance
      conics/distance={#1,#2,\cs@fdist}, 
    }
    \tikzmath{
      ...
      if abs(\cs@c-\cs@a) <= \EPSILON then {
        ...
        if abs(\cs@c) <= \EPSILON then {% circle
          ...
          \cs@e = \fpeval{\cs@c/\cs@a};% 离心率 Eccentricity
        };
      };
    }
    ...
    \ifnum\fpeval{\cs@c > \EPSILON ? 1: 0} = 1% \ifnum 不能直接判断大小
      \tikzmath{
        % 连接两焦点的方向
        \cs@fdirect{1} = \fpeval{(#2{1,1})/(#2{3,1})-(#1{1,1})/(#1{3,1})};
        \cs@fdirect{2} = \fpeval{(#2{2,1})/(#2{3,1})-(#1{2,1})/(#1{3,1})};
        % 与两焦点连线垂直的方向
        \cs@fperp{1} = -\cs@fdirect{2};
        \cs@fperp{2} = \cs@fdirect{1};
        % 系数
        \cs@s = \fpeval{-0.5/(\cs@e)^2};
      }
      \tikzset{
        % translation matrix
        conics/translation matrix={\cs@s*\cs@fdirect{1},\cs@s*\cs@fdirect{2},\cs@TM},
        mat/product={\cs@TM,\cs@center,3,3,1,\cs@P},
        % mat/stdout={\cs@TM,3,3},
        % mat/stdout={\cs@P,3,1},
        conics/translation matrix={\cs@fperp{1},\cs@fperp{2},\cs@TP},
        mat/product={\cs@TP,\cs@P,3,3,1,\cs@Q},
        conics/cross={\cs@P,\cs@Q,\cs@directrix},
        conics/define/focus-directrix={#1,\cs@directrix,\cs@e,#4},
      }
    \else%circle
      \tikzset{
        conics/define/circle/center-radius={#1,#3,#4},
      }
    \fi
  },
  ...
  conics/define/ellipse/foci-point/.code args={#1,#2,#3,#4}{
    \tikzset{
      conics/distance={#3,#1,\cs@da},
      conics/distance={#3,#2,\cs@db},
      conics/define/foci-axis={#1,#2,\fpeval{0.5*(\cs@da+\cs@db)},#4},
    }
  },
  conics/define/hyperbola/foci-point/.code args={#1,#2,#3,#4}{
    \tikzset{
      conics/distance={#3,#1,\cs@da},
      conics/distance={#3,#2,\cs@db},
      conics/define/foci-axis={#1,#2,\fpeval{0.5*abs(\cs@da-\cs@db)},#4},
    }
  },
  % Four points in general position (i.e., no three collinear) 
  % form the basis of a pencil of conics.
  % conics/define/@basic={\Pa,\Pb,\Pc,\Pd,\Ca,\Cb},
  conics/define/@basis/.code args={#1,#2,#3,#4,#5,#6}{
    \tikzset{
      ...
      mat/outer product={\cs@Lab,\cs@Lcd,3,#5},
      mat/outer product={\cs@Lac,\cs@Lbd,3,#6},
    }
  },
  % Pencil of conics through 4 points
  % C = lab * lcd^T + lcd * lab^T + k * (lac * lbd^T + lbd * lac^T)
  % conics/define/pencil={\Pa,\Pb,\Pc,\Pd,\k,\C}
  % \Pi: 4 points, homogeneous coordinates, dimension 3x1
  % \C: the coefficient matrix, dimension 3x3
  % \C-non-degenerate conic
  conics/define/pencil/.code args={#1,#2,#3,#4,#5,#6}{
    \tikzset{
      conics/define/@basis={#1,#2,#3,#4,\cs@Ca,\cs@Cb},
      mat/combine={\cs@Ca,\cs@Cb,3,3,1,#5,\cs@M},
  ...
  conics/define/five points/.code args={#1,#2,#3,#4,#5,#6}{
    \tikzset{
      conics/define/@basis={#1,#2,#3,#4,\cs@Ca,\cs@Cb},
      mat/bilinear={\cs@Cb,#5,#5,3,\cs@fa},
      mat/bilinear={\cs@Ca,#5,#5,3,\cs@fb},
      mat/combine={\cs@Ca,\cs@Cb,3,3,\cs@fa,(-\cs@fb),\cs@M},
      mat/transpose={\cs@M,3,3,\cs@N},
      mat/add={\cs@M,\cs@N,3,3,#6},
    }
  },
  ...
  conics/rotate/.code args = {#1,#2,#3,#4} {
    \tikzmath {
      if abs(#1{1,2}) > \EPSILON then {
        ...
        % perform rotation
        #2{1,1} = \fpeval{
          #1{1,1}*(\cs@cos)^2+#1{1,2}*2*\cs@sin*\cs@cos+#1{2,2}*(\cs@sin)^2
        };
        #2{1,2} = 0;
        #2{1,3} = \fpeval{#1{1,3}*\cs@cos+#1{2,3}*\cs@sin};
        #2{2,2} = \fpeval{
          #1{1,1}*(\cs@sin)^2-#1{1,2}*2*\cs@sin*\cs@cos+#1{2,2}*(\cs@cos)^2
        };
        ...
        #2{3,2} = #2{2,3};
        % rotation matrix
        #3{1,1} = \cs@cos; #3{1,2} =-\cs@sin; #3{1,3} = 0;
        #3{2,1} = \cs@sin; #3{2,2} = \cs@cos; #3{2,3} = 0;
        #3{3,1} = 0;       #3{3,2} = 0;       #3{3,3} = 1;
        #4 = atan(\cs@tan);
      } else {
        % unchanged
        int \cs@i, \cs@j;
        ...
        % rotation matrix
        #3{1,1} = 1; #3{1,2} = 0; #3{1,3} = 0;
        #3{2,1} = 0; #3{2,2} = 1; #3{2,3} = 0;
        #3{3,1} = 0; #3{3,2} = 0; #3{3,3} = 1;
        #4 = 0;
      };
    }%tikzmath
  ...
  % \P,\Q: intersections (homogeneous coordinates, dimension 3x1, normalized)
  conics/intersect cl/.code args={#1,#2,#3,#4,#5}{
    ...
    \tikzmath{
      % {\message{a,b,c:\cs@a,\cs@b,\cs@c^^J}};
      \cs@max = max(abs(\cs@a),abs(\cs@b),abs(\cs@c));
      \cs@a = \fpeval{\cs@a/\cs@max};
      \cs@b = \fpeval{\cs@b/\cs@max};
      \cs@c = \fpeval{\cs@c/\cs@max};
      % solve: a m^2+2 b m n+c n^2 = 0, return m/n
      if abs(\cs@a) <= 100*\EPSILON && abs(\cs@c) <= 100*\EPSILON then {% a,c=0
        ...
        if abs(\cs@a) > 100*\EPSILON then {% a != 0, solve m/n
    ...
    \tikzset{
      mat/combine={#2,#3,3,1,\cs@ma,\cs@na,\cs@pa},
      mat/combine={#2,#3,3,1,\cs@mb,\cs@nb,\cs@pb},
      mat/normalize={\cs@pa,3,1,#4},
      mat/normalize={\cs@pb,3,1,#5},
    }
  },
  ...
  conics/rotation matrix/.code args={#1,#2}{
    \tikzmath{
      #2{1,1} = cos(#1); #2{1,2} = -sin(#1); #2{1,3} = 0;
      #2{2,1} = sin(#1); #2{2,2} = cos(#1);  #2{2,3} = 0;
  ...
  conics/pole/.code args={#1,#2,#3}{
    \tikzset{
      ...
      mat/product={\cs@dual,#2,3,3,1,\cs@pole},
      mat/normalize={\cs@pole,3,1,#3},
    }
    % \tikzmath{
  ...
  conics/intersect cn/.code args={#1,#2,#3,#4}{
    ...
    \tikzset{
      mat/combine={\cs@B,\cs@ss,3,3,1,\cs@alpha,\cs@C},
      mat/decompose rank-1={\cs@C,3,3,\cs@pa,\cs@pb},
      mat/normalize={\cs@pa,3,1,#3},
      mat/normalize={\cs@pb,3,1,#4},
    }
  ...
  conics/intersect cc/.code args={#1,#2,#3,#4,#5}{
    ...
    \message{a,b,c,lambda:\cs@a,\cs@b,\cs@c,\cs@lambda^^J}

    \tikzset{
      mat/combine={#1,#2,3,3,\cs@lambda,1,#3},
  ...
  conics/skew-symmetric matrix/.code args={#1,#2}{
    \tikzmath{
      #2{1,1} = 0;        #2{1,2} = -#1{3,1}; #2{1,3} = #1{2,1};
      #2{2,1} = #1{3,1};  #2{2,2} = 0;        #2{2,3} = -#1{1,1};
      #2{3,1} = -#1{2,1}; #2{3,2} = #1{1,1};  #2{3,3} = 0;
    }
  },
  % Richter-Gebert. Perspective on Projective Geometry. Springer,2011.
  ...
  % Given two conics C1 and C2 intersecting at four points (A,B,C,D), 
  % if the line through two points (e.g., AB) is known, the function 
  % returns the line through the remaining two points (CD) in the 
  % conic pencil C1 + λ C2 = 0.
  % conics/linear factor={\Ca,\Cb,\lab,\P,\lcd}
  % \Ca,\Cb: conics, 3x3 matrix
  % \lab: line coefficients, 3x1 matrix
  % \P: any point on \lab but not on C1 and C2
  % returns \lcd: line coefficients, 3x1 matrix
  conics/linear factor/.code args={#1,#2,#3,#4,#5}{
    \tikzset{
      mat/bilinear={#1,#4,#4,3,\cs@a},
      mat/bilinear={#2,#4,#4,3,\cs@b},
      mat/combine={#1,#2,3,3,1,-(\cs@a/\cs@b),\cs@Cdeg},
      conics/decompose={\cs@Cdeg,\cs@l,\cs@m},
      conics/exclude={\cs@l,\cs@m,#3,#5},
    }
  },
}
...
% 主命令: 负责接收可选参数 [] 和圆括号内容 ()
\def\segment[#1,clip=(#2:#3,#4:#5)]#6(#7){
  % 在单个参的解析方式上, 当你在 \segment 中使用 (\l) 时，LaTeX 
  % 期望 #7 是一个带括号的列表，但 \l 本身包含逗号且没有大括号保护。
  % 直接使用 pgfutil@for 会出错: Argument of \l has an extra }.
  \pgfutil@in@{,}{#7}
  \ifpgfutil@in@% 多个参数
    % 计算 #7 中参数的个数
    \pgfkeyssetvalue{/tikz/conics/segment/@argn}{0} % 重置计数器
    \pgfutil@for\arg:=#7\do{
    \pgfmathparse{int(add(\pgfkeysvalueof{/tikz/conics/segment/@argn},1))}
    \pgfkeysalso{/tikz/conics/segment/@argn = \pgfmathresult}
    }
    \message{segment command argn:\pgfkeysvalueof{/tikz/conics/segment/@argn}^^J}
    \ifnum \pgfkeysvalueof{/tikz/conics/segment/@argn} = 2
    % 处理双点(列向量 3x1)的宏
    \segment@points[#1,clip=(#2:#3,#4:#5)](#7)%
    \else\ifnum \pgfkeysvalueof{/tikz/conics/segment/@argn} = 3
    % 处理直线系数方程: a,b,c
    \segment@coefs[#1,clip=(#2:#3,#4:#5)](#7)%
    \else
    \pgferror{Incorrect number of arguments for 'segment'}
    \fi\fi
  \else%单个参数
    % 处理直线系数(列向量 3x1)的宏
...
\def\segment@points[#1,clip=(#2:#3,#4:#5)]#6(#7,#8){%
  \tikzset{conics/cross={#7,#8,\segment@l},mat/stdout={\segment@l,3,1}}
  \path[#1,conics/segment={#2,#3,#4,#5,\segment@l}]
}

% 辅助宏: 处理 \segment[options](a,b,c)
\def\segment@coefs[#1,clip=(#2:#3,#4:#5)]#6(#7,#8,#9){%
  \tikzmath{
    \segment@l{1,1} = #7;
    \segment@l{2,1} = #8;
    \segment@l{3,1} = #9;
