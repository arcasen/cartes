\ProvidesFile{tikzlibrarycartes.code.tex}[2026/05/08 v1.0.1 A tikz library for geometry computations]
...
\def\parabola[#1]#2(#3,#4,#5){
  \path[smooth,domain=-2:2,#1,variable=\cates@var]
    plot({\cates@var},
      {(#3)*\cates@var*\cates@var+(#4)*\cates@var+(#5)})
}
...
\def\hyperbola[#1]#2(#3,#4){
  \path[smooth,domain=-1.5:1.5,#1,variable=\cates@var]
    plot({-(#3)*cosh(\cates@var)},
      {(#4)*sinh(\cates@var)})% right arm
    plot({ (#3)*cosh(\cates@var)},
      {(#4)*sinh(\cates@var)})% left arm
}
...
\def\asymptote[#1]#2(#3,#4){
  \path[domain=-2:2,#1,variable=\cates@var,]
    plot({\cates@var},{ (#4)/(#3)*(\cates@var)})  
    plot({\cates@var},{-(#4)/(#3)*(\cates@var)}) 
}
