...
\begin{tikzpicture}
  ...
  \tikzset{
    ...
    plane/midpoint={\A,\D,\Ma},
    plane/midpoint={\C,\F,\Mb},
    plane/midpoint={\B,\E,\Mc},
  }
  ...
  \coordinate (M1) at (\Ma{1},\Ma{2});
  \coordinate (M2) at (\Mb{1},\Mb{2});
  \coordinate (M3) at (\Mc{1},\Mc{2});

  \draw[thick,navy] (A) -- (B) -- (D) -- (F) (A) -- (C);
  \draw[thick,red] ($(P1)!-0.25!(P2)$) -- ($(P1)!1.25!(P2)$);
  \draw[thick,violet] ($(H2)!-0.25!(H4)$) -- ($(H2)!1.25!(H4)$);
  \draw[thick,purple] ($(M1)!-0.25!(M3)$) -- ($(M1)!1.25!(M3)$);
  ...
  \foreach \p/\placement in {M1/above,M2/above,M3/above}{
    \fill[orange] (\p) circle (1.5pt);
  }

  \draw[xshift=-2.5cm,yshift=-5.5cm] node [right,text width=9cm,style=information text]
  {
    \textit{完全四边形的三条对角线段中点共线，记作为 Newton线。Newton 线与 Gauss 线和 Steiner 线垂直。}
  };
