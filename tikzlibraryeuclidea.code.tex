%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% DEPRECATION WARNING: This library is unmaintained.  
%% No security patches or feature updates will be made.  
%% Please migrate to `tikzlibrarycates.code.tex`(2026-05-01).  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\ProvidesFile{tikzlibraryeuclidea.code.tex}[2026/05/01 v1.8.2 A tikz library for Euclidean geometry constructions]
...
\tikzset{
  ...
  % circle mat/inverse={O,A,P}: returns inverse point P with respect to 
  ...
  revolve/angle/.code = {%
    ...
    \else
      ...
      \pgfkeyssetvalue{/tikz/revolve/@argn}{0} % 重置计数器
  ...
  intersect/eqs/.code args = {#1,#2,#3,#4,#5,#6}{
    ...
    \tikzset{mat/solve={\intersect@a,\intersect@b,2,1,\intersect@ans},}
    \typeout{=========================}
    \typeout{Line-Line Intersection:}
    \tikzset{
      mat/stdout={\intersect@a,2,2},
      mat/stdout={\intersect@b,2,1},
      mat/stdout={\intersect@ans,2,1},
    }
    \typeout{=========================}
  ...
  conic/rotate/.code args = {#1,#2,#3}{
    \tikzmath {
      if abs(#1{1,2}) > \EPSILON then {
        ...
        if \cs@tau >= 0 then {
          \cs@t = \fpeval{1.0 / (\cs@tau + sqrt(1+(\cs@tau)^2))};
        } else {
          \cs@t = \fpeval{-1.0 / (- \cs@tau + sqrt(1+(\cs@tau)^2))};
        };
  ...
  conic/translate/.code args = {#1,#2,#3,#4}{
    \tikzset{
      mat/cofactors={#1,3,\cs@cofactors},
    }
    \tikzmath{
  ...
  conic/reduce/.code args = {#1,#2,#3,#4,#5}{
    \tikzset {
      ...
      conic/translate={\cs@qr,\cs@qt,\cs@xshift,\cs@yshift},
    }
    \tikzmath {
      ...
      #4 = \cs@xshift * cos(#3) - \cs@yshift * sin(#3);
      #5 = \cs@xshift * sin(#3) + \cs@yshift * cos(#3);
    }%tikzmath
  },
