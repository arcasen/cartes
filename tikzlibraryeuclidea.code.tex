\ProvidesFile{tikzlibraryeuclidea.code.tex}[2026/02/28 v1.8.1 A tikz library for Euclidean geometry constructions]
...
%\usepackage{ifthen} % Required for string comparison

% Syntax: \axes[grid] (xmin:xmax, ymin:ymax)
% If you don't need the grid, simply use: \axes (xmin:xmax, ymin:ymax)
\newcommand\axes[1][]{%
  \axesinternal[#1]%
}

\def\axesinternal[#1](#2:#3,#4:#5){
  % 1. Check if the "grid" option is provided
